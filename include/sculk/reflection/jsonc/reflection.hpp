...
bool load_file(T& t, const std::filesystem::path& path, const F& key_formatter, const options& options = {}) {
    ...
    if (options.rewrite == rewrite_policy::always || (options.rewrite == rewrite_policy::error && !result)
        || options.rewrite == rewrite_policy::format) {
        ...
        if (options.rewrite == rewrite_policy::format && content) {
