...
template <concepts::is_least_stringifiable_type T>
constexpr std::optional<T> string_to_type(std::string_view sv, bool enum_case_sensitive) {
    ...
    if constexpr (traits::is_string_convertible_v<T>) {
        ...
        if (auto val = enum_case_sensitive ? magic_enum::enum_cast<RT>(sv) : magic_enum::enum_cast<RT>(sv, magic_enum::case_insensitive)) {
            return val;
        }
        if (auto val =
                enum_case_sensitive ? magic_enum::enum_flags_cast<RT>(sv) : magic_enum::enum_flags_cast<RT>(sv, magic_enum::case_insensitive)) {
            return val;
        }
