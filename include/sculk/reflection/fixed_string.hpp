...
template <size_t N>
struct std::formatter<sculk::reflection::fixed_string<N>> : std::formatter<std::string_view> {
    template <typename FormatContext>
    auto format(const sculk::reflection::fixed_string<N>& str, FormatContext& ctx) const {
...
template <size_t N>
struct fmt::formatter<sculk::reflection::fixed_string<N>> : fmt::formatter<std::string_view> {
    template <typename FormatContext>
    auto format(const sculk::reflection::fixed_string<N>& str, FormatContext& ctx) const {
        return formatter<std::string_view>::format(str.view(), ctx);
    }
