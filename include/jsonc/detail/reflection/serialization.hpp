...
constexpr JsoncType serialize_impl(const T& t, const Options& options, KeyFormatter kfmt, PriorityTag<10>) noexcept;

template <concepts::is_boolean_type T>
constexpr JsoncType serialize_impl(const T& t, const Options& options, KeyFormatter kfmt, PriorityTag<9>) noexcept;
...
constexpr JsoncType serialize_impl(const Renamed<T, N>& t, const Options& options, KeyFormatter kfmt, PriorityTag<8>) noexcept;

template <typename T, FixedString... Args>
constexpr JsoncType serialize_impl(const Annotated<T, Args...>& t, const Options& options, KeyFormatter kfmt, PriorityTag<7>) noexcept;

template <concepts::is_dispatcher T>
constexpr JsoncType serialize_impl(const T& t, const Options& options, KeyFormatter kfmt, PriorityTag<6>) noexcept;

template <concepts::is_optional T>
constexpr JsoncType serialize_impl(const T& t, const Options& options, KeyFormatter kfmt, PriorityTag<6>) noexcept;

template <concepts::is_ranged T>
constexpr JsoncType serialize_impl(const T& t, const Options& options, KeyFormatter kfmt, PriorityTag<5>) noexcept;

template <concepts::is_enum T>
constexpr JsoncType serialize_impl(const T& t, const Options& options, KeyFormatter kfmt, PriorityTag<5>) noexcept;

template <concepts::is_variant T>
constexpr JsoncType serialize_impl(const T& t, const Options& options, KeyFormatter kfmt, PriorityTag<4>) noexcept;

template <concepts::is_string_convertible T>
constexpr JsoncType serialize_impl(const T& t, const Options& options, KeyFormatter kfmt, PriorityTag<3>) noexcept;

template <concepts::is_tuple_like T>
constexpr JsoncType serialize_impl(const T& t, const Options& options, KeyFormatter kfmt, PriorityTag<3>) noexcept;

template <concepts::is_array_like T>
constexpr JsoncType serialize_impl(const T& t, const Options& options, KeyFormatter kfmt, PriorityTag<2>) noexcept;

template <concepts::is_associative T>
constexpr JsoncType serialize_impl(const T& t, const Options& options, KeyFormatter kfmt, PriorityTag<2>) noexcept;

template <concepts::is_aggregate T>
constexpr JsoncType serialize_impl(const T& t, const Options& options, KeyFormatter kfmt, PriorityTag<1>) noexcept;

template <typename T>
constexpr JsoncType serialize_impl(const T& t, const Options& options, KeyFormatter kfmt, PriorityTag<0>) noexcept;
...
template <typename T>
[[nodiscard]] constexpr JsoncType serialize(const T& t, KeyFormatter key_formatter, const Options& options = {}) noexcept {
    return serialize_impl(t, options, key_formatter, PriorityTag<10>{});
}

namespace {

template <concepts::is_arithmetic T>
constexpr JsoncType serialize_impl(const T& t, const Options&, KeyFormatter, PriorityTag<10>) noexcept {
    return t;
}

template <concepts::is_boolean_type T>
constexpr JsoncType serialize_impl(const T& t, const Options&, KeyFormatter, PriorityTag<9>) noexcept {
...
template <typename T, FixedString N>
constexpr JsoncType serialize_impl(const Renamed<T, N>& t, const Options& options, KeyFormatter kfmt, PriorityTag<8>) noexcept {
    return serialize_impl(*t, options, kfmt, PriorityTag<10>{});
}

template <typename T, FixedString... Args>
constexpr JsoncType serialize_impl(const Annotated<T, Args...>& t, const Options& options, KeyFormatter kfmt, PriorityTag<7>) noexcept {
    auto result = serialize_impl(*t, options, kfmt, PriorityTag<10>{});
...
template <concepts::is_optional T>
constexpr JsoncType serialize_impl(const T& t, const Options& options, KeyFormatter kfmt, PriorityTag<6>) noexcept {
    if (t) { return serialize_impl(*t, options, kfmt, PriorityTag<10>{}); }
    return nullptr;
}
...
template <concepts::is_enum T>
constexpr JsoncType serialize_impl(const T& t, const Options& options, KeyFormatter, PriorityTag<5>) noexcept {
...
template <concepts::is_variant T>
constexpr JsoncType serialize_impl(const T& t, const Options& options, KeyFormatter kfmt, PriorityTag<4>) noexcept {
    return std::visit([&](const auto& val) { return serialize_impl(val, options, kfmt, PriorityTag<10>{}); }, t);
}

template <concepts::is_string_convertible T>
constexpr JsoncType serialize_impl(const T& t, const Options&, KeyFormatter, PriorityTag<3>) noexcept {
...
constexpr JsoncType serialize_impl(const T& t, const Options& options, KeyFormatter kfmt, PriorityTag<3>) noexcept {
    ...
    std::apply(
        [&](const auto&... args) { (([&](const auto& v) { res.push_back(serialize_impl(v, options, kfmt, PriorityTag<10>{})); }(args)), ...); },
        t
    );
    return res;
}
...
constexpr JsoncType serialize_impl(const T& t, const Options& options, KeyFormatter kfmt, PriorityTag<2>) noexcept {
    JsoncType res = JsoncType::array();
    for (const auto& val : t) { res.push_back(serialize_impl(val, options, kfmt, PriorityTag<10>{})); }
    return res;
}
...
constexpr JsoncType serialize_impl(const T& t, const Options& options, KeyFormatter kfmt, PriorityTag<2>) noexcept {
    ...
    for (const auto& [key, val] : t) { res[detail::type_to_string(key)] = serialize_impl(val, options, kfmt, PriorityTag<10>{}); }
    return res;
}
...
template <typename T>
constexpr JsoncType serialize_impl(const T&, const Options&, KeyFormatter, PriorityTag<0>) noexcept {
